...
const StateManager = {
  ...
  set: (state) =>
    localStorage.setItem("xrashxourseState", JSON.stringify(state)),
  update: (id, value) => {
...
function getOperationDay() {
  ...
  const day =
    Math.floor((today - parseInt(startDate)) / (1000 * 60 * 60 * 24)) + 1;
...
async function initApp() {
  try {
    const [dataRes, dsaRes] = await Promise.all([
      fetch("data.json"),
      fetch("dsa.json")
    ]);
...
function renderActiveDay() {
  ...
  gsap.fromTo(
    ".view.active .bento-card, .view.active .task-item",
    { y: 15, opacity: 0 },
...
function renderForge(d, forgeDay, state) {
  document.getElementById("forge-subtitle").innerHTML =
    `<i class="ph ph-fire"></i> Day ${d}: ${forgeDay.title}`;
  let html = "";
...
function renderDSA(d, dsaDay, state) {
  document.getElementById("dsa-subtitle").innerHTML =
    `<i class="ph ph-code-block"></i> Day ${d}: ${dsaDay.title}`;
...
function bindGlobalEvents() {
  ...
  document.querySelectorAll(".nav-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (targetView) {
        ...
        gsap.fromTo(
          `#${targetId} .header h1, #${targetId} .header .subtitle`,
          { y: 15, opacity: 0 },
  ...
  document
    ...
    .addEventListener("click", () => toggleProf(true));
  document
    .getElementById("close-profile")
    .addEventListener("click", () => toggleProf(false));
  document
    .getElementById("profile-overlay")
    .addEventListener("click", () => toggleProf(false));
...
function calculateMetrics() {
  ...
  for (let d = 1; d <= 5; d++) {
    ...
    globalData.forge.days[d - 1].tasks.forEach((t, i) => {
      ...
      metrics.daily[d].t++;
      if (state[id]) {
        metrics.mission.completed++;
        metrics.daily[d].c++;
      }
      if (t.topics)
        t.topics.forEach((sub, sIdx) => {
          const sId = `${id}-s-${sIdx}`;
          metrics.mission.total++;
          metrics.daily[d].t++;
          if (state[sId]) {
    ...
    // DSA Topics
    const allTopics = [
      ...globalData.dsa.days[d - 1].topics,
      ...globalData.dsa.days[d - 1].patterns
    ];
...
function updateProgress() {
  ...
  const overallPct =
    metrics.mission.total > 0
      ? Math.round((metrics.mission.completed / metrics.mission.total) * 100)
      : 0;

  // Rings and Bars
  const circle = document.getElementById("avatar-progress");
  if (circle)
    circle.style.strokeDashoffset = 138.2 - (overallPct / 100) * 138.2;
  ...
  // Stars
  document.getElementById("star-bar-fill").style.width =
    `${(metrics.stars.completed / metrics.stars.total) * 100}%`;
  document.getElementById("star-percentage").textContent =
    `${metrics.stars.completed} / ${metrics.stars.total}`;
  document.getElementById("header-star-count").textContent =
    metrics.stars.completed;
  ...
  const exportObj = {
    ...
    missionProgress: {
      ...
      completionPercentage: Math.round(
        (metrics.mission.completed / metrics.mission.total) * 100
      )
    },
    commendations: {
